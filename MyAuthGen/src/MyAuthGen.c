...
int main(int argc, char **argv)
{
    ...
    if (rc522_read_card_id(curr_id, &card_tipe))
    {
        ...
        //login to block 1
        if(mfrc522_auth(littlewire, PICC_AUTHENT1A, 1, keyA_default, curr_id) != 1);
        return -1;
        ...
        {
            //login to block 2
            if(mfrc522_auth(littlewire, PICC_AUTHENT1A, 2, keyA_default, curr_id) != 1);
            return -1;
            Status = mfrc522_write_block(littlewire, 2, DataBufL);
        }
    }

    mfrc522_halt(littlewire);
